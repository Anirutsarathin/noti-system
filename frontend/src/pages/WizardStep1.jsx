...
//             avgWindowMin: Number(p.avg_window_min || 60),
//           },
...
export default function WizardStep1({ mode }) {
  ...
  useEffect(() => {
    const run = async () => {
      if (mode === 'create') {
        ...
        // ✅ default mode สำหรับ create (กัน undefined)
        // - latest: avgWindowMin = 0
        // - avg: default 60 นาที (จะถูก set ตอนเลือก avg)
        updateTargets({
          dataMode: draft?.targets?.dataMode || 'latest',
          avgWindowMin: draft?.targets?.avgWindowMin ?? 0,
        })
        return
      }
      ...
      try {
        ...
        const isAvg = Number(p.avg_window_min || 0) > 0
        ...
        setFromExisting({
          ...
          targets: {
            ...
            // ✅ เลือกข้อมูลที่จะใช้ส่ง: latest vs avg
            // 👉 ปรับใหม่: ถ้าเลือก avg -> fix 60 นาทีเสมอ
            dataMode: isAvg ? 'avg' : 'latest',
            avgWindowMin: isAvg ? AVG_DEFAULT_MIN : 0,
          },
          sensors: draft.sensors.map(s => {
            const found = (p.sensors || []).find(x => x.key === s.key)
            return { ...s, rules: found ? found.rules : (s.rules || []) }
  ...
  const next = () => {
    ...
    // 3) data mode (latest/avg) -> fix avg = 60 นาที, latest = 0
    const dataMode = draft.targets.dataMode || 'latest'
    if (dataMode === 'avg') {
      updateTargets({ avgWindowMin: AVG_DEFAULT_MIN })
    } else {
      updateTargets({ avgWindowMin: 0 })
    }
        ...
        >
          ...
          >
            <input
              ...
              name="dataMode"
              checked={dataMode === 'latest'}
              onChange={() => updateTargets({ dataMode: 'latest', avgWindowMin: 0 })}
            />
            ค่าล่าสุด
          </label>

          {/* AVG (fix 60 นาที, ไม่มีให้กรอก) */}
          <label
            className="pill"
            style={{
              cursor: 'pointer',
          ...
          >
            <input
              ...
              name="dataMode"
              checked={dataMode === 'avg'}
              onChange={() => updateTargets({ dataMode: 'avg', avgWindowMin: AVG_DEFAULT_MIN })}
            />
            ค่าเฉลี่ย (คงที่ {AVG_DEFAULT_MIN} นาที)
          </label>
        </div>

        <div className="small" style={{ marginTop: 8 }}>
          {dataMode === 'avg'
            ? `ระบบจะใช้ค่าเฉลี่ยย้อนหลัง ${AVG_DEFAULT_MIN} นาที ก่อนส่ง`
            : 'ใช้ค่าล่าสุด 1 แถวจากตารางสถานี '}
        </div>
