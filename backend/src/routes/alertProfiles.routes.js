...
//     `SELECT sd.code as station_code
//       FROM alert_profile_station a
//       JOIN station_details sd
//       ON a.station_code COLLATE utf8mb4_general_ci = sd.name COLLATE utf8mb4_general_ci
//       WHERE a.profile_id=? 
//       ORDER BY station_code`,
//     [profileId]
//   );
...
async function loadProfile(profileId) {
  ...
  // ✅ รองรับทั้งเก็บ code หรือเก็บ name
  const [st] = await db.query(
    `
...
router.post("/:id/send-now", async (req, res) => {
  try {
    ...
    // ✅ ส่งจริงผ่าน profileSender (มี logic: ส่งเฉพาะผิดปกติ + กันซ้ำ)
    await sendProfileNow(id);
